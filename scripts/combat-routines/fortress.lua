...
function Routine_BallistaMoveFirst(side, hero, level)
    -- print("Trigger ballista play first !")
    SetATB_WarMachineType(side, WAR_MACHINE_BALLISTA, ATB_INSTANT)
    COMBAT_PAUSE = 0
end

function Routine_SkirmishersRandomShoot(side, hero, level)
...
function Routine_ThanesAbility(side, hero, level)
...
function Routine_CastFireWalls(side, hero, level)
...
function Routine_RunePriestsMoveFirst(side, hero, level)
...
function Routine_SummonEarthElementals(side, hero, level)
...
function Routine_CastMeteorShowers(side, hero, level)
...
function Routine_BallistaUseHalfATB(side, hero, level)
...
function Routine_CastRandomImplosion(side, hero, level)
...
function DoFortressRoutine_CombatPrepare(side, name, id, level)
    startThread(FORTRESS_COMBAT_PREPARE[name], side, id, level)
end

function DoFortressRoutine_CombatStart(side, name, id, level)
    startThread(FORTRESS_COMBAT_START[name], side, id, level)
end

function DoFortressRoutine_CombatTurn(side, name, id, level)
    startThread(FORTRESS_COMBAT_TURN[name], side, id, level)
end

function DoFortressRoutine_UnitDied(side, name, id, level, unit)
    startThread(FORTRESS_UNIT_DIED[name], side, id, level, unit)
end
