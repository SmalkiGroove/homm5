...
function ApplyHeroesSpe_daily(player)
	...
	if heroes~=nil then
		...
		if contains(heroes,"RedHeavenHero06") ~= nil then Spe_AddCreatures2("RedHeavenHero06",player,110,0.13) end; -- Zealot - 1:4 - 2:12 - 3:20 - 4:27 ... 7:50
		-- Preserve
...
function ApplyHeroesSpe_weekly(player)
	...
	if heroes~=nil then
		-- Haven
		if contains(heroes,"Nathaniel") ~= nil then Spe_AddRecruits("Nathaniel",player,CREATURE_PEASANT,TOWN_BUILDING_DWELLING_1,7) end;
		if contains(heroes,"Ving") ~= nil then Spe_CallCreatures("Ving",player,CREATURE_GRIFFIN,TOWN_BUILDING_DWELLING_4,1.5) end;
